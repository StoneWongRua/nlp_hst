...
from ch4.spam.clean_data import *
from ch4.spam.preprocess_data import *
...
class BayesSpam(object):
  ...
  def _count(self):
    ...
    if self.user_data:
      print("加载用户定义的正常邮件")
      my_ham_list = os.listdir(os.path.join(self.meta_path,r'C:\Users\15845\SpamBayes\spam\myham'))
      for file in my_ham_list:
        with open(os.path.join(self.meta_path, os.path.join(r'C:\Users\15845\SpamBayes\spam\myham', file)), 'r', encoding='utf-8') as f:
          unique_word_set = set()
          self.ham_count += 1
          while True:
            line = f.readline()
      ...
      print("加载用户定义的垃圾邮件")
      my_spam_list = os.listdir(os.path.join(self.meta_path, r'C:\Users\15845\SpamBayes\spam\myspam'))
      for file in my_spam_list:
        with open(os.path.join(self.meta_path, os.path.join(r'C:\Users\15845\SpamBayes\spam\myspam', file)), 'r', encoding='utf-8') as f:
          unique_word_set = set()
          self.spam_count += 1
          while True:
            line = f.readline()
...
if __name__ == "__main__":
  ...
  file_list = get_all_files(r'C:\Users\15845\trec06c\data\000')
  print("合计有效文件：", len(file_list))
  spam_set = get_index(r'C:\Users\15845\trec06c\full\index')
  ...
  for file_i in test_file_list:
    ...
    if bs.test_email(file_i) == is_spam(file_i, spam_set):
      correct += 1
      if is_spam(file_i, spam_set): # 判断file是在垃圾邮件集合里面
          print("1" ,is_spam(file_i, spam_set), file_i, "不是垃圾邮件，预测正确！")
      else:
          print("2", is_spam(file_i, spam_set), file_i, "是垃圾邮件，预测正确！")
    else: # file 不在垃圾邮件集合里面
      print("False")
      if is_spam(file_i, spam_set):
        print(is_spam(file_i, spam_set),file_i, "是垃圾邮件，预测错误！")
      else:
        print(is_spam(file_i, spam_set),file_i, "不是垃圾邮件，预测错误！")
  end = time()
