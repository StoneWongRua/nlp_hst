from ch6.spam.spamEmail import spamEmailBayes
import re
# spam类对象
spam=spamEmailBayes()
# 保存词频的词典
...
for fileName in normFileList:
    wordsList.clear()
    for line in open(r"C:\Users\15845\data\normal\\"+fileName):
        #过滤掉非中文字符
...
for fileName in spamFileList:
    wordsList.clear()
    for line in open(r"C:\Users\15845\data\spam\\"+fileName):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
...
for fileName in testFileList:
    ...
    wordsDict.clear()
    wordsList.clear()
    for line in open(r"C:\Users\15845\data\test\\"+fileName):
        rule=re.compile(r"[^\u4e00-\u9fa5]")
        line=rule.sub("",line)
        spam.get_word_list(line,wordsList,stopList)
    spam.addToDict(wordsList, wordsDict)
